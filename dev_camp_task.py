...
class Layer:
            ...
            for t, k in enumerate(data_keys_1_occ):
                ...
                if key_dat_k == self.layer[l_0 - 1][c_0]:
                    ...
                    if self.check_for_overlay(l_0, c_0, index, key_dat_k):
                        ...
                        self.check_line_1(l_0 - 1, c_0)
                        self.database[index][key_dat_k]["times_occurrence"] = 0
                        self.database[index][key_dat_k]["coordinates"] = []
                        self.check_line_2(l_0, c_0)
    ...
    def move_in(self):
        """Modifying the work database"""

        self.layer = [[False for _ in range(self.column)]
                      for _ in range(self.line)]
        self.count = 0
        self.database.append(self.database.pop(0))
        for i, el in enumerate(self.database):
            for key in el.keys():
                self.database[i][key]["coordinates"] = []
                self.database[i][key]["times_occurrence"] = 0
    ...
    def __repr__(self):
        """Processing the graphical expression of
                the first solution in the combination library"""
            ...
            for i in range(1, len(middle_line), 2):
                if not i % 2 == 0:
                    if not middle_line[i] == middle_line[i - 1]:
                        ...
                        columns.append(" " * self.max_chars)
            for i in range(0, len(middle_line), 2):
                if i == 0:
                    continue
                if i % 2 == 0:
                    if not middle_line[i] == middle_line[i - 2]:
                        rows.append("*")
                    else:
                        rows.append(" ")
            row = "*"
